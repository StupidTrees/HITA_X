...
class TimetableRepository(val application: Application) {
    ...
    fun actionNewTimetable() {
        Thread {
            val defaultTables = timetableDao.getTimetableNamesWithDefaultSync(application.getString(R.string.default_timetable_name) + "%")
            var max = 0
            ...
            newTable.name = application.getString(R.string.default_timetable_name) + (max + 1).toString()
